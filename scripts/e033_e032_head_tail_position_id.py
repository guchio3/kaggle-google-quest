...
class QUESTDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        position_ids = idx_row['position_ids']  # .sqeeeze()
    ...
    def _trim_input(self, title, question, answer,
        ...
        if (t_len + q_len + a_len + 4) > self.MAX_SEQUENCE_LENGTH:
            ...
            # # 長さを揃える
            t_new_len = min(t_new_len, 512)
            q_new_len = min(q_new_len, 512)
            a_new_len = min(a_new_len, 512)
            ...
            if len(question) > q_new_len:
                ...
                question_position_ids = [i for i in range(q_new_len // 2)] \
                    + [i for i in range(512 - (q_new_len // 2), 512)]
            else:
                question_position_ids = [i for i in range(len(question))]
            if len(answer) > a_new_len:
                answer = answer[:a_new_len // 2] + answer[-a_new_len // 2:]
                answer_position_ids = [i for i in range(a_new_len // 2)] \
...
#        category = row.category
        ...
        title, body, answer, position_ids = self._trim_input(title, body, answer,
                                                             t_max_len=t_max_len,
                                                             q_max_len=q_max_len,
                                                             a_max_len=a_max_len)
        ...
        # position_ids = [i for i in range(len(title))] + \
        #     [i for i in range(len(body))] + \
        #     [i for i in range(len(answer))]
        # position_ids += [0] * (whole_len - len(position_ids)) # padding
        position_ids = torch.tensor(position_ids)
        encoded_texts_dict['position_ids'] = position_ids
