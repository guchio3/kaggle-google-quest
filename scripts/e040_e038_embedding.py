...
class QUESTDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return qa_id, input_ids, attention_mask, \
            token_type_ids, cat_labels, position_ids, host_le, labels
...
#        category = row.category
        ...
        host_map = {
                'host_stackexchange': 0,
                'host_askubuntu': 1,
                'host_mathoverflow': 2,
                'host_serverfault': 3,
                'host_stackoverflow': 4,
                'host_superuser': 5,
            }
        host_le = host_map[row.host]

        # category を text として入れてしまう !!!
        if self.use_category:
            title = [category] + title
            # title = [category] + [host] + title
        ...
        encoded_texts_dict['host_le'] = host_le
...
class BertModelForBinaryMultiLabelClassifier(nn.Module):
    ...
    def forward(self, input_ids=None, input_cats=None, labels=None, attention_mask=None,
                token_type_ids=None, position_ids=None, host_les=None, head_mask=None,
        ...
        emb = self.emb(host_les)
        pooled_output = torch.cat([pooled_output, emb], dim=1)
...
def train_one_epoch(model, fobj, optimizer, loader):
    ...
    for (qa_id, input_ids, attention_mask,
         token_type_ids, cat_labels, position_ids, host_les, labels) in tqdm(loader):
        # send them to DEVICE
...
def main(args, logger):
    ...
    for HOST in HOSTs:
        trn_df.loc[trn_df.host.str.contains(HOST).values, 'host'] = f'HOST_{HOST}'.casefold()
