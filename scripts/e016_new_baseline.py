...
class QUESTDataset(Dataset):
    def __init__(self, df, mode, tokens, augment,
        ...
        tokens = [token.encode('ascii', 'replace').decode()
                  for token in tokens if token != '' and type(token) == str]
        print(tokens)
...
def main(args, logger):
    ...
    for fold, (trn_idx, val_idx) in enumerate(gkf):
        ...
        logger.info('calurating new tokens ...')
        # temp_tokenizer = BertTokenizer.from_pretrained(TOKENIZER_PRETRAIN, do_lower_case=True)
        # temp = pd.Series(list(itertools.chain.from_iterable(
        #     fold_trn_df.question_title.apply(lambda x: temp_tokenizer.tokenize(x)) +
        #     fold_trn_df.question_body.apply(lambda x: temp_tokenizer.tokenize(x)) +
        #     fold_trn_df.answer.apply(lambda x: temp_tokenizer.tokenize(x))
        # ))).value_counts()
        # tokens = temp[temp >= 3].index.tolist()
        # tokens += [
