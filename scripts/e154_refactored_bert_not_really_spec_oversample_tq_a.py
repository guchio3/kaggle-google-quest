...
def main(args, logger):
    ...
    for fold, (trn_idx, val_idx) in enumerate(gkf):
        ...
        os_fold_trn_df = fold_trn_df[
                    (fold_trn_df.question_not_really_a_question > 0).values |
                    (fold_trn_df.question_not_really_a_question > 0).values
                ]
        fold_trn_df = pd.concat([fold_trn_df], axis=0)
        fold_trn_df = pd.concat([os_fold_trn_df] * 10 + [fold_trn_df, raw_pseudo_df, opt_pseudo_df, half_opt_pseudo_df], axis=0)
