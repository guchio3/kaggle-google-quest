...
class QUESTDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return qa_id, input_ids, attention_mask, \
            token_type_ids, cat_labels, labels
...
def train_one_epoch(model, fobj, optimizer, loader):
    ...
    for (qa_id, input_ids, attention_mask,
         token_type_ids, cat_labels, labels) in tqdm(loader):
        # send them to DEVICE
...
def main(args, logger):
    ...
    for fold, (trn_idx, val_idx) in enumerate(gkf):
        ...
        del model
    sel_log(f'val_metric_mean : {np.mean(np.mean(val_metrics)):.5f}', logger)
    sel_log(f'val_metric_std : {np.mean(np.std(val_metrics)):.5f}', logger)
