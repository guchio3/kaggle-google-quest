...
class QUESTDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        text_lengths = idx_row['text_lengths'].squeeze()

        labels = self.labels.iloc[idx].values
        return qa_id, input_ids, attention_mask, \
            token_type_ids, cat_labels, position_ids, text_lengths, labels
...
#        category = row.category
        ...
        org_title_len = len(title)
        org_body_len = len(body)
        org_answer_len = len(answer)
        org_title_len = (org_title_len - 12.259088665898997) / 5.486062367603494
        org_body_len = (org_body_len - 227.47573614081264) / 336.9704269293547
        org_answer_len = (org_answer_len - 216.35170258266163) / 287.85266791201167
        ...
        encoded_texts_dict['text_lengths'] = torch.tensor(
            [org_title_len, org_body_len, org_answer_len])
...
class BertModelForBinaryMultiLabelClassifier(nn.Module):
    def __init__(self, num_labels, pretrained_model_name_or_path=None,
        ...
        if cat_num > 0:
            ...
            self.dropout = nn.Dropout(0.2)
            self.classifier = nn.Linear(
                self.model.pooler.dense.out_features+3, num_labels)
        self.length_adapter = nn.Linear(
            3, 3)
        self.add_module('fc_output', self.classifier)
        self.add_module('fc_output_2', self.length_adapter)
    ...
    def forward(self, input_ids=None, input_cats=None, labels=None, attention_mask=None,
                token_type_ids=None, position_ids=None, text_lengths=None, head_mask=None,
        ...
        pooled_output = torch.cat([pooled_output, text_lengths.float()], dim=1)
...
def train_one_epoch(model, fobj, optimizer, loader, pair_fobj=None):
    ...
    for (qa_id, input_ids, attention_mask,
         token_type_ids, cat_labels, position_ids, text_lengths, labels) in tqdm(loader):
        # send them to DEVICE
