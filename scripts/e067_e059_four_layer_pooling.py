...
class BertModelForBinaryMultiLabelClassifier(nn.Module):
    ...
    def forward(self, input_ids=None, input_cats=None, labels=None, attention_mask=None,
        ...
        pooled_output_12 = torch.mean(outputs[2][-1], dim=1)
        pooled_output_11 = torch.mean(outputs[2][-2], dim=1)
        pooled_output_10 = torch.mean(outputs[2][-3], dim=1)
        pooled_output_9 = torch.mean(outputs[2][-4], dim=1)
        # pooled_output = torch.mean(outputs[0], dim=1)
        pooled_output = pooled_output_12 + pooled_output_11 \
            + pooled_output_10 + pooled_output_9
